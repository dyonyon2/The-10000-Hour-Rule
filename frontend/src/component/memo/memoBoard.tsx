...
export default function MemoBoard() {
    ...
    async function changeFavorite(target:MemoListInfo){
        ...
        if(result.res_status){  // 정상적인 API CALL Return
            if(result.res_status==="1") {  // 메모 읽기
                console.log("메모 즐겨찾기 수정 성공");
                const newMemoList:MemoListInfo[] = memoList.map((memo:MemoListInfo) =>
                     memo.memo_idx === target.memo_idx
                        ?{...memo, favorites: target.favorites} as MemoListInfo
                        :memo
                );
                SetMemoList(newMemoList); // API Call 성공시에 변경
            } else {
                console.log("메모 즐겨찾기 수정 실패");
                alert("메모 즐겨찾기 수정 실패");
            } 
        } else {    // 비정상적인 API CALL Return
            alert("올바르지 않은 Response 입니다."+result);
        ...
        <>
                            ...
                            <Grid2 container size={12} justifyContent={"space-around"} alignContent={"space-around"}>
                                {
                                    (memoList.slice((page-1)*parseInt(sortCnt)*parseInt(sortCnt),page*parseInt(sortCnt)*parseInt(sortCnt))).map((memo, idx) => (
                                        ...
                                        <MemoBoardItem style={sortCnt=="3"?style.memoItem3x3:(sortCnt=="4"?style.memoItem4x4:style.memoItem5x5)} data={memo} changeFavorite={changeFavorite}/>
                                        :<></>
                                    ))
                                }
                            </Grid2>
            ...
            <Grid2 container size={12} justifyContent={"space-between"} alignContent={"space-around"} >
                ...
                <Grid2 container size={3} justifyContent={"flex-end"} alignContent={"space-around"} >
                    <Button type="submit" fullWidth variant="text" className={style.icon_square} onClick={()=>{}}><Create />+</Button>
                </Grid2>
            </Grid2>
        </>
    );
